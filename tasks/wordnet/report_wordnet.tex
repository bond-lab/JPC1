...
At first, I decided to go in a different direction, as there were some unexpected difficulties in importing chainnet into wordnet (the \texttt{wn} module did not yet support the \lnk{metaphor} and \lnk{metonym} links, neither did the \texttt{wn-edit} module).

But \texttt{wn} version 0.14 supports  \lnk{metaphor} and \lnk{metonym}, so I wrote a new \texttt{wn\_edit} module and imported ChainNet to the English Wordnet, both for the OMW English Wordnet, and an updated Open English Wordnet. 
...
The wn\_edit module\footnote{\url{https://github.com/bond-lab/wn_edit}}  extends the `wn` module with capabilities for creating, modifying, and exporting WordNet lexicons. It works directly with `wn.lmf` data structures, ensuring full compatibility with the `wn` ecosystem.
...
\subsection*{Importing tropes to Wordnet}

We used the new \texttt{wn\_edit} module to create a version of the English wordnet that contains the metaphor and metonymy links.

The script \texttt{enhance.py}\footnote{Available at \url{https://github.com/fcbond/ChainNet}} does the following:
\begin{enumerate}
\item Load an English Wordnet (the default is \texttt{omw-en:1.4})
\item Loads and parses the chainnet data (\texttt{chainnet\_\{metaphor|metonymy\}.json})
\item Creates an index from the sensekey to the sense id
\item Adds the metaphor and metonym links and their reverses to the
  wordnet (\lnk{metaphor}, \lnk{has\_metaphor},
  \lnk{metonym}, \lnk{has\_metonym})
\item Saves it with a new version (the default is \texttt{omw-en:1.4.cn})
\end{enumerate}

The new wordnet can now be used to investigate tropes:
\begin{scriptsize}
\begin{verbatim}
>>> import wn
...
Sense('omw-en-chestnut-12262553-n') {'metonym': [Sense('omw-en-chestnut-12262905-n')], 
                                     'has_metonym': [Sense('omw-en-chestnut-07772274-n')]}
Sense('omw-en-chestnut-07772274-n') {'metaphor': [Sense('omw-en-chestnut-02468504-n')], 
                                      'metonym': [Sense('omw-en-chestnut-12262553-n'),
                                                  Sense('omw-en-chestnut-04972350-n')]}
Sense('omw-en-chestnut-04972350-n') {'derivation': [Sense('omw-en-chestnut-00373209-s')], 
                                     'has_metonym': [Sense('omw-en-chestnut-07772274-n')], 
                                     'metonym': [Sense('omw-en-chestnut-02388735-n')]}
...
There are several other measures it would be nice to look at: inclusiveness (describe in Section~\ref{sec:approach}), the domain from \href{https://adimen.ehu.eus/web/XWND}{eXtended WordNet Domains} \citep{10.1007/978-3-642-28604-9_2} and sentiment.


The wordnet merge script is has not been extensively tested, but works with the latest versions of the Open Multilingual Wordnet  and the Open English Wordnet.  It uses \texttt{wn.compat.sensekey} to look up senses with sense keys.  66 relations that were in \texttt{omw-en:1.4} are no longer in \texttt{oewn:2024}.\footnote{For example fro \textit{horseman, hacker, jihad, annulet} and \textit{charade}.}
